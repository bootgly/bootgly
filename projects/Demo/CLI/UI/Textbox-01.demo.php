<?php
namespace Bootgly\CLI;

use function preg_match;

use const Bootgly\CLI;
use Bootgly\CLI\UI\Components\Textbox;

$Input = CLI->Terminal->Input;
$Output = CLI->Terminal->Output;
$Output->reset();

$Output->render(<<<OUTPUT
/* @*:
 * @#green: Bootgly CLI UI - Textbox component @;
 * @#yellow: @@: Demo 27 - Example #1 - single input with validation @;
 * {$location}
 */\n\n
OUTPUT);

// @ Textbox with a default (press Enter to accept it)
$Textbox = new Textbox($Input, $Output);
$Textbox->prompt = 'Server port';
$Textbox->default = '8080';
$Textbox->Validator = static function (string $answer): true|string {
   // ?:
   if (preg_match('#^\d{1,5}$#', $answer) !== 1) {
      return 'Invalid port: use a number between 1 and 65535.';
   }

   // :
   return true;
};

$port = $Textbox->ask();

$Output->render("@.;Port accepted: @#cyan:{$port}@;@..;");

// @ Required answer (re-asks on empty answers; 3 attempts)
$Textbox = new Textbox($Input, $Output);
$Textbox->prompt = 'Project name';
$Textbox->required = true;
$Textbox->attempts = 3;
$Textbox->default = 'MyApp';

$name = $Textbox->ask();

$Output->render("@.;Project name: @#green:{$name}@; (attempt {$Textbox->attempt})@..;");

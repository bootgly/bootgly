...
return new Test(
   ...
   test: function () use ($host, $port) {
      ...
      try {
         ...
         // ! Remove every query before the INSERT under test. Tokens::mint()
         //   normally revokes first, while both stores may probabilistically
         //   sweep; neither may consume the alarm or hide the target write.
         ...
         $Tokens = new class ($TokenDatabase, $tokens) extends Tokens {
            /** Disable supersession so mint() reaches its INSERT directly. */
            public function revoke (string $user, null|Purposes $Purpose = null): int
            {
               return 0;
            }

            /** Disable probabilistic cleanup in this single-write fixture. */
            public function sweep (): int
            {
               return 0;
            }
         };
         $Trust = new class ($TrustDatabase, $trusts) extends Trust {
            /** Disable probabilistic cleanup in this single-write fixture. */
            public function sweep (): int
            {
               return 0;

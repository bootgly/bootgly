...
class Decoder_HTTP2 extends Decoders implements Disconnecting, Feeding
{
   ...
   protected function resolve (Packages $Package, int $stream, string $block): null|States
   {
      ...
      foreach ($fields as [$name, $value]) {
         ...
         // ? Field names must be lowercase in HTTP/2 (RFC 9113 §8.2.1)
      ...
      if ($method === null || $method === '' || $scheme === null || $target === null || $target === '') {
         return $this->reset($stream, Errors::Protocol);
      }
      // ? Extended CONNECT is not supported (WS-over-h2 out of scope)
      if ($method === 'CONNECT') {
         $this->reset($stream, Errors::RefusedStream);
         return null;
      }
    ...
    */
   protected function deny (int $stream, int $code): null|States
   {
      $this->outbox .= Frame::pack(
         HTTP2::FRAME_HEADERS,
         HTTP2::FLAG_END_HEADERS | HTTP2::FLAG_END_STREAM,
         $stream,
         HPACK::encode([[':status', (string) $code]])
      );
   ...
   protected function pump (Packages $Package): void
   {
      ...
      foreach ($this->Streams as $id => $Stream) {
         ...
         $portion = substr($Stream->backlog, 0, $send);
         ...
         for ($offset = 0; $offset < $send; $offset += $limit) {
            $chunk = substr($portion, $offset, min($limit, $send - $offset));
            $this->outbox .= Frame::pack(
               HTTP2::FRAME_DATA,

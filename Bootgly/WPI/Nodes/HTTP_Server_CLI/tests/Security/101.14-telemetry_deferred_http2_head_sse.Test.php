...
/**
 * L4 deferred HTTP/2 HEAD+SSE ownership regression.
 *
 * Response::defer() attaches its Cancellation generation to the live HTTP/2
 * Stream before starting user work. SSE::open() then closes that Stream while
 * producing a HEAD response. The 200 lifecycle must be selected before Stream
 * teardown can report cancellation: exactly one HEADERS+END_STREAM frame, one
 * 2xx terminal status and no duplicate or status-less terminal are permitted.
 */
$Probe = new class {
...
return new Test(
   description: 'Deferred HTTP/2 HEAD SSE must select 200 before Stream cancellation',
   Separator: new Separator(line: true),

   requests: [
      static fn (): string => "GET /l4/deferred-h2-head-sse/setup HTTP/1.1\r\n"
         . "Host: localhost\r\n\r\n",
      ...
      static function (string $hostPort, int $testIndex) use ($Probe): string {
         try {
            ...
            if ($Socket === false) {
               throw new RuntimeException(
                  "L4 deferred h2 HEAD SSE connection failed: {$errorNumber} {$errorMessage}"
               );
            }
            ...
            $headers = HPACK::encode([
               [':method', 'HEAD'],
               [':scheme', 'http'],
               [':path', '/l4/deferred-h2-head-sse/target'],
               [':authority', 'localhost'],
            ...
            if ($offset !== strlen($request)) {
               throw new RuntimeException(
                  'L4 deferred h2 HEAD SSE request was not sent completely.'
               );
            }
            ...
            while (microtime(true) < $deadline) {
               ...
               // ! Continue beyond the first terminal frame so a stale normal
               //   encoder write cannot escape the exactly-once assertion.
               if ($terminalAt !== null && microtime(true) - $terminalAt >= 0.25) {
                  break;
               }
            }
            fclose($Socket);
         ...
         return "GET /l4/deferred-h2-head-sse/evidence HTTP/1.1\r\n"
            . "Host: localhost\r\n\r\n";
      },
   ],
   ...
   response: static function (
      ...
      yield $Router->route('/l4/deferred-h2-head-sse/setup', static function (
         Request $Request,
         Response $Response,
      ) use ($Probe): Response {
         ...
         return $Response(body: 'L4-DEFERRED-H2-HEAD-SSE-SETUP');
      }, GET);

      yield $Router->route('/l4/deferred-h2-head-sse/target', static function (
         Request $Request,
         Response $Response,
      ) use ($Probe): Response {
         $Selected = $Response->defer(static function (Response $Deferred) use (
            ...
            // ! The stale application status distinguishes terminal selection
            //   by SSE (200) from either normal deferred output (409) or a
            //   status-less Stream cancellation.
            $Deferred->code(409);
            $SSE = $Deferred->SSE;
            $SSE->heartbeat = 0;
            $SSE->open();
            $Probe->closed = $SSE->closed;
         });
         $Probe->deferred = $Selected->deferred;

         return $Selected;
      }, HEAD);

      yield $Router->route('/l4/deferred-h2-head-sse/evidence', static function (
         Request $Request,
         Response $Response,
         ...
         return $Response(body: 'L4-DEFERRED-H2-HEAD-SSE:' . json_encode($evidence));
      }, GET);
   },
   ...
   test: static function (array $responses) use ($Probe): bool|string {
      if (
         ...
         || str_contains(
            $responses[0] ?? '',
            'L4-DEFERRED-H2-HEAD-SSE-SETUP',
         ) === false
      ) {
         return 'L4 deferred h2 HEAD SSE setup/harness response failed.';
      }
      if ($Probe->error !== '') {
         return 'L4 deferred h2 HEAD SSE fixture failed: ' . $Probe->error;
      }
      ...
      if (
         ...
         || ($streamFrames[0]['flags'] & HTTP2::FLAG_END_HEADERS) === 0
         || ($streamFrames[0]['flags'] & HTTP2::FLAG_END_STREAM) === 0
      ) {
         return 'L4 deferred h2 HEAD SSE emitted DATA, duplicate frames, or no '
            . 'terminal HEADERS on stream 1: ' . json_encode($streamFrames);
      }

      $HPACK = new HPACK(4096);
      $fields = [];
      foreach (
         $HPACK->decode($streamFrames[0]['payload'], PHP_INT_MAX) ?? []
         as [$name, $value]
      ) {
         $fields[$name] = $value;
      }
      if (
         ...
         || ($fields['content-type'] ?? null) !== 'text/event-stream'
         || isSet($fields['content-length'])
      ) {
         return 'L4 deferred h2 HEAD SSE did not emit the canonical 200 SSE '
            . 'metadata head: ' . json_encode($fields);
      }

      $wire = $responses[1] ?? '';
      $separator = strpos($wire, "\r\n\r\n");
      $body = $separator === false ? '' : substr($wire, $separator + 4);
      $prefix = 'L4-DEFERRED-H2-HEAD-SSE:';
      ...
      if ($evidence !== $expected) {
         return 'L4 deferred h2 HEAD SSE did not close exactly once with the '
            . 'wire-selected 2xx status before Stream cancellation: '
            . json_encode([
               'expected' => $expected,
               'actual' => $evidence,
               'frames' => $streamFrames,

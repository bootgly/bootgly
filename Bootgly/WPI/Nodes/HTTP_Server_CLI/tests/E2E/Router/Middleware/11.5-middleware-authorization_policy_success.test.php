...
return new Specification(
   description: 'It should authorize a real HTTP request with JWT and Policy middleware',
   ...
   response: function (Request $Request, Response $Response): Response {
      return $Response(body: 'authorized:' . $Request->identity->id);
   },

   test: function ($response) {
      return str_contains($response, 'HTTP/1.1 200 OK')
         && str_contains($response, 'authorized:e2e-user')
            ?: 'Authorization Policy middleware did not pass the granted request';
   }
);
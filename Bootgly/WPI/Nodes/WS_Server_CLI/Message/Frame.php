...
/**
 * A single RFC 6455 frame (§5.2) and its byte codec.
 */
final class Frame
{
   // * Data
   ...
   private static function unmask (string $payload, string $key): string
   {
      $length = strlen($payload);
      if ($length === 0) {
         return '';
      ...
      $mask = str_repeat($key, intdiv($length, 4) + 1);

      return $payload ^ substr($mask, 0, $length);
   }
}

...
class SetupCommand extends Command
{
   ...
   private function install (string $scriptName, string $installDir, string $installPath): bool
   {
      ...
      //   `opcache.jit` is not writable at runtime (ini_set() returns false),
      //   so a missing php.ini setting is unrecoverable once PHP has started.
      //   Bootgly is a long-lived CLI runtime — its hottest functions run
      //   fully interpreted without the tracing JIT, which reads as "the
      //   framework is slow" rather than "the runtime was misconfigured".
      //   BOOTGLY_JIT=0 opts out; the native code-coverage driver needs it.
      $wrapper = "#!/bin/bash\n"
         ...
         . "   exec \"$phpBinary\" -d opcache.jit=disable \"$scriptPath\" \"\$@\"\n"
         . "fi\n"
         . "exec \"$phpBinary\""
         . " -d opcache.enable=1"

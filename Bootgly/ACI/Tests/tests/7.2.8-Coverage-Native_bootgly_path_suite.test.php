...
return new Specification(
   description: 'Coverage — Native reports the Path suite through the bootgly CLI',
   ...
   test: new Assertions(Case: function (): Generator {
      ...
      if (! is_resource($process)) {
         throw new RuntimeException('Could not run Native coverage Path suite probe.');
      }
      ...
      yield (new Assertion(description: 'nested bootgly coverage command exits cleanly'))
         ->expect($code)
         ->to->be(0)
         ->assert();
      ...
      yield (new Assertion(description: 'Path suite native coverage has executable denominator'))

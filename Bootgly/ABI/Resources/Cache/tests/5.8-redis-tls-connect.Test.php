...
return new Test(
   description: 'Cache(Redis): a TLS connect on the native lane survives the socket tuning',
   ...
   test: function () use ($PHP) {
      ...
      try {
         ...
         //   CACHE-13: socket_import_stream() cannot represent a TLS stream — it warns,
         ...
         // # …and the write actually crossed the TLS wire
         ...
         foreach ($lines as $line) {
            if (str_starts_with((string) $line, '1|SET probe ')) {
               $crossed = true;
            }
         }
